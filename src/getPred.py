...
model = load_model(f'./model/model-batch_size-64-learning_rate-0.001.h5')
model.summary()
x_scaler = joblib.load('./other/scaler/x_scaler.sav')
y_scaler = joblib.load('./other/scaler/y_scaler.sav')
...
list = os.listdir('./data/Data_Test')
...
# splitting the predictions into 300 parts with 12 elements in each
l = 1
...
for i in range(len(predictions)):
    if i % 12 == 0:
        predictions_split_temp = predictions[i:i+12]
...
with open(F'./test.csv', 'w',  newline='') as csvfile:
